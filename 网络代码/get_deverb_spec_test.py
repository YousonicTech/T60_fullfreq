# -*- coding: utf-8 -*-
"""
@file      :  get_deverb_spec_test.py
@Time      :  2022/9/15 13:53
@Software  :  PyCharm
@summary   :  Test for getting FPN clean image
@Author    :  Bajian Xiang
"""
# -*- coding: utf-8 -*-
...
from data_load_for_FPN_spec import Dataset_dict, collate_fn
...
def parse_args():
    ...
    parser.add_argument('--epoch_for_save', type=int, default=3)  # 记得改epoch数
    ...
    parser.add_argument('--outputresult_dir', type=str, default="./0915_test/epoch3")  # 记得改输出路径，不用带/应该
...
def test_net(net, epoch, val_loader, writer, batch_size):
    ...
    with torch.no_grad():
        ...
        for j, datas in enumerate(progress_bar):
            ...
            names = datas['name']
            print('---- Test sample:', names[0][0])

            gt_t60_reshape = gt_t60_reshape.T[args.freq_loc].T  # [total_slices, 1]
            output_pts, dereverb_img = net(images_reshape)
            ...
            # TODO 整一个新的记录dict
            if names[0][0] not in result_image_dict.keys():
                save_name = names[0][0]
                result_image_dict[save_name] = {}
                result_image_dict[save_name]['clean'] = clean_reshape
                result_image_dict[save_name]['image'] = images_reshape
                result_image_dict[save_name]['dereverb'] = dereverb_img
                result_image_dict[save_name]['t60_gt'] = gt_t60_reshape
                result_image_dict[save_name]['t60_pred'] = output_pts
                result_image_dict[save_name]['ssim'] = ssim(dereverb_img, clean_reshape)
        ...
        return result_dict, result_image_dict
...
if __name__ == "__main__":
    ...
    if not DEBUG:
        ...
        val_dict_root = "/data/xbj/0902_1000hz_with_clean/val"
        val_batch_size = 1
    ...
    test_output_result, result_image_dict = test_net(net, trained_epoch, val_loader, None, val_batch_size)
    ...
    image_save_path =  save_path = outputresult_dir + '/image_result_epoch' + str(args.epoch_for_save) + '.pt'
    torch.save(result_image_dict, image_save_path)